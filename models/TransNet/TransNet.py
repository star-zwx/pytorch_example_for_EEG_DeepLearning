...
class TransNet(nn.Module):
    def __init__(self, num_classes=4, num_samples=640, num_channels=25, embed_dim=32, pool_size=50,
        ...
        # self.classify = nn.Linear(embed_dim*16, num_classes)
...
def main():
    input = torch.randn(32, 25, 640)
    model = TransNet(num_classes=4)
    out = model(input)
    print('===============================================================')
    print('out', out)
    print('model', model)
    summary(model=model, input_size=(25, 640), batch_size=32, device="cpu")
